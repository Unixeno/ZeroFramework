...
class DebugCore {
    ...
    public static function errorHandler($errno, $errstr, $errfile, $errline) {
        if ($errno == E_USER_ERROR) {
            array_push(self::$message_list, ['type' => 'E_USER_ERROR',
                'message' => $errstr,
                'file'    => $errfile,
                'line'    => $errline
            ]);
            self::errorPrinter(True);
            exit(1);
        } elseif ($errno == E_ERROR) {
            array_push(self::$message_list, ['type' => 'E_ERROR',
                'message' => $errstr,
                'file'    => $errfile,
                'line'    => $errline
            ]);
        ...
        array_push(self::$message_list, ['type' => $type,
            'message' => $errstr,
            'file'    => $errfile,
            'line'    => $errline
        ]);
    ...
    public static function errorPrinter($printStake = false) {
        ...
        $message = array_pop(self::$message_list);
        while ($message != NULL) {
